...
ipv6_host_policy_egress(struct __ctx_buff *ctx, __u32 srcID, __u32 *dstID) {
...
# if !defined(ENABLE_HOST_SERVICES_FULL) || \
     (defined(ENABLE_EXTERNAL_IP) && !defined(BPF_HAVE_NETNS_COOKIE))
	{
		...
		/* 148991 insns before, 114907 after */
		relax_verifier();
	}
...
ipv6_host_policy_ingress(struct __ctx_buff *ctx, __u32 *srcID) {
	struct ct_state ct_state_new = {}, ct_state = {};
	__u8 policy_match_type = POLICY_MATCH_NONE;
	int ret, ret2, verdict, l4_off, hdrlen;
...
#ifdef ENABLE_HOST_FIREWALL
	if (from_host)
		ret = ipv6_host_policy_egress(ctx, secctx, &remoteID);
	else
...
ipv4_host_policy_ingress(struct __ctx_buff *ctx, __u32 *srcID) {
	int ret, ret2, verdict, l4_off, l3_off = ETH_HLEN;
	struct ct_state ct_state_new = {}, ct_state = {};
	__u8 policy_match_type = POLICY_MATCH_NONE;
	__u32 monitor = 0, dstID = WORLD_ID;
	struct remote_endpoint_info *info;
...
static __always_inline int
do_netdev_(struct __ctx_buff *ctx, __u16 proto, bool from_host)
{
	__u32 __maybe_unused identity = 0;
	int trace = TRACE_FROM_HOST, ret;
	bool from_proxy;
...
int do_netdev(struct __ctx_buff *ctx, bool from_host)
{
	int ret = ret;
	__u16 proto;
	...
	if (!from_host) {
		cilium_dbg_capture(ctx, DBG_CAPTURE_SNAT_PRE,
				   ctx_get_ifindex(ctx));
		ret = snat_process(ctx, BPF_PKT_DIR);
		if (ret != CTX_ACT_OK) {
			return ret;
		}
		cilium_dbg_capture(ctx, DBG_CAPTURE_SNAT_POST,
				   ctx_get_ifindex(ctx));
	}
#endif /* ENABLE_MASQUERADE */

	return do_netdev_(ctx, proto, from_host);
}
...
# ifdef ENABLE_IPV6
	case bpf_htons(ETH_P_IPV6):
		...
		ret = ipv6_host_policy_egress(ctx, srcID, &remoteID);
		break;
# endif
# ifdef ENABLE_IPV4
	case bpf_htons(ETH_P_IP):
