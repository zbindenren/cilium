...
var (
	...
	defaultHelmOptions = map[string]string{
		...
		// Disable by default, so that 4.9 CI build does not panic. On 4.19 and
		// net-next we enable it with kubeProxyReplacement=strict.
...
func (kub *Kubectl) WaitPodDeleted(pod string) error {
	body := func() bool {
		_, cancel := context.WithTimeout(context.Background(), ShortCommandTimeout)
		defer cancel()
		res := kub.GetPods(DefaultNamespace, pod)
		return !res.WasSuccessful()
	}
